...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/v1/admindashboard');
      if (!response.ok) {
  ...
  if (error) {
    return (
      <div className="admindashboard-error">
        <h2>Error</h2>
        <p>{error}</p>
            ...
            {data.map((item, index) => (
              <div key={item.id || index} className="data-item">
                ...
                <div className="item-actions">
                  <button onClick={() => navigate(`/admindashboard/${item.id}`)}>
                    View
                  </button>
                  <button onClick={() => navigate(`/admindashboard/${item.id}/edit`)}>
                    Edit
                  </button>
                </div>
              </div>
